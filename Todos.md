...
**التحديد عبر POST فقط — حذف account/stores (Phase 41):**
1. **حذف صفحة `/merchant/account/stores`:** route `stores` من `routes/merchant.php:35`، الفولتين `resources/views/livewire/merchant/stores/index.blade.php`، و`GetStoreCardsAction.php` + `StoreCardData.php` (كانا حصريين للصفحة)، وكود ميت: `ChooseStoreController::index` + `resources/views/auth/choose-store.blade.php`.
2. **إعادة توجيه روابط القائمة الخمسة إلى `merchant.choose-store`:** topbar:39 (store-link)، store-sidebar:101 (store-switch)، merchant-sidebar:34 (my_stores + routeIs)، account-sidebar:33 (+ routeIs)، user-dropdown:6 — كلها تصير صفحة قائمة المتاجر الوحيدة.
3. **التحديد POST فقط في choose-store:** زر «فتح» صار `<form method="POST" action="{{ route('merchant.choose-store.select', $store['slug']) }}" class="contents">` + `@csrf` (نمط صفحة account/stores المحذوفة نفسها → `ChooseStoreController::select` بحراسات المجالين) وحذف Closure `selectStore` + استيراد `StoreContext`.
4. **إغلاق باكدور الرابط:** `StoreResolver::resolve` — فحص `X-Store-Id`/`?store_id` مقصور على `/api/*` فقط (عقد API عبر `EnsureStoreContext` يبقى سليمًا)؛ لا يمكن لأي GET/رابط في لوحة التاجر تحديد المتجر.
5. **الاختبار:** `BillingVisibilityScopingTest` §4 → ① `/merchant/account/stores` = 404 ② GET `/merchant/choose-store` ينجح ولا يعيّن `current_store_id` ③ POST `merchant.choose-store.select` يعيّنها (مع `_token` = csrf_token) ويعيد توجيه الداشبورد.
6. **الشهادة:** `php -l` نظيف (routes/merchant.php، StoreResolver، ChooseStoreController، الاختبار)؛ `view:clear` + `route:clear` ناجحان؛ `route:list` يؤكد اختفاء `account.stores` وبقاء `merchant.choose-store` GET + `merchant.choose-store.select` POST؛ greps صفرية لـ `account.stores` (في القوالب) و`merchant.stores.index|GetStoreCardsAction|StoreCardData|auth.choose-store`؛ `BillingVisibilityScopingTest` **6/6 (27 تأكيد)** + `EmailVerificationTest` ناجح. المسح الكامل (Auth+Account+Merchant) = **641 ناجح + 38 فشل سابقة الوجود** (انحراف مهاجرة/DB للبيئة + spatie cache) مؤكَّدة على baseline نظيف عبر `git stash` (عينات: SplitLogin merchant login، OrderTracking QueryException، RoleScopingTest:79) — **صفر انحدارات من هذا التغيير**.
7. **المرونة 375/768/1440:** التغيير البصري الوحيد غلاف `class="contents"` (نفس نمط الصفحة المحذوفة) داخل `flex` أوامر البطاقة — بلا تغيير في المواصفات؛ شبكة البطاقات `grid-cols-1 md:grid-cols-2` (١ عمود موبايل / ٢ عند ≥768) كما هي؛ روابط topbar/sidebar/dropdown غيّرنا الـ URL فقط دون markup → لا أثر عند النقاط الثلاث.
