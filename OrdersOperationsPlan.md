...
### 29.3 — الإرسال الجماعي للطلبيات مجمَّعًا حسب شركة كل طلبية (تم بموافقة المستخدم)

- **الفجوة:** الإرسال الجماعي القديم (`bulkSendToCarrier($providerId)`) كان يدفع كل المحددة إلى شركة واحدة يختارها المستخدم — مخالفًا لربط كل طلبية بشركتها/موصّلها، ولا يعرض ملخصًا للتأكيد، ولا يحترم فحص الجهوزية.
- **الحل:** ثلاث إجراءات جديدة واستبدال زر «إرسال» القديم:
  - `$openBulkSendModal()` — حارس `order.manage` + تحذير `no_orders_selected`؛ يقرأ المحددة ويجمّعها بـ`shipping_provider_id` (بديل `delivery_rider_id`)؛ الطلبيات بلا شركة وموصّل → مجموعة «بلا شركة»؛ ويملأ `bulkSendSummary` (اسم الشركة + عدد) مخصصًا `x-edz.modal`.
  - `$collectMissingFields(Order)` — مغلف ذكي لخمسة حقول جهوزية (اسم/هاتف/ولاية/بلدية/عنوان-أو-نقطة/صنف/شركة-أو-موصّل) مستخرَج من 29.2 ويعاد استخدامه هناك وفي الجماعي.
  - `$confirmBulkSend()` — يرسل كل طلبية لشركتها عبر `OrderShippingGateway::send(order, providerId: $order->shipping_provider_id ?: null, changedBy)` (لا تأكيد تلقائي أبدًا)؛ غير المؤكَّد/الناقص يُتخطَّى دون تغيير حالته؛ توست واحد يجمع سطور الشركات + عدد المتخطَّى (أيقونة تحذير إن وُجد تخطٍّ، نجاح بخلافه).
- **UI:** زر واحد «إرسال لشركة التوصيل» في شريط الإجراءات المشترك (يستبدل قائمة الشركات السابقة) + مودال تأكيد بملخص لكل شركة. درج «تأكيد وإرسال» (P26) لم يُعدَّل.
- **ترجمات:** مفاتيح 9 ×4 لغات في `order_flow.php`: `bulk_send_summary_title`/`bulk_send_summary_subtitle`/`bulk_send_group_count`/`bulk_send_no_groups`/`bulk_send_confirm`/`bulk_send_unassigned`/`bulk_send_rider`/`bulk_send_summary_line`/`bulk_send_skipped`.
- **ملاحظة اختبار:** `assertDispatched('swal:toast', closure)` يفحص أول حدث مطابق فقط — لذا صُهر التوستان السابقان في حدث واحد.
- **أدلة:** `tests/Feature/Merchant/BulkSendCarrierGroupingTest.php` — **8 ناجحة (31 assertions)** (منع بلا صلاحية، تحذير بدون تحديد، التجميع حسب الشركة/الموصّل/بلا شركة، إرسال كل طلبية لشركتها مع ملخص، تخطِّي pending دون تأكيد تلقائي، تخطِّي الناقص، إرسال طلبية موصّل فقط، تخطِّي بلا شركة نهائيًا). السويت كاملة **313 ناجح (1131 assertions)** → **صفر انحدار**. `view:cache` + `php -l` سليمان.
