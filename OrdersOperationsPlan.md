...
- **أدلة:** `tests/Feature/Merchant/DirectSendConfirmedOrderTest.php` — **7 ناجحة (16 assertions)** (منع بلا صلاحية بتوست `messages.unauthorized` — كان 403 ونُقِّح في إغلاق 29.7، رفض pending دون تأكيد تلقائي، إرسال confirmed/preparing → shipped مع حدث `sent_to_carrier`، منع بعنوان ناقص مع ذِكر الحقل، حصر كل الحقول الناقصة، قبول نقطة استلام بدل العنوان). السويت كاملة **305 ناجح (1100 assertions)** → **صفر انحدار** (نجح CartOrderLimitsTest ضمن السويت). `view:cache` + `php -l` سليمان.

---

### 29.3 — الإرسال الجماعي للطلبيات مجمَّعًا حسب شركة كل طلبية (أُعيد العمل عليه بملاحظات المستخدم)

- **الفجوة:** الإرسال الجماعي القديم (`bulkSendToCarrier($providerId)`) كان يدفع كل المحددة إلى شركة واحدة يختارها المستخدم — مخالفًا لربط كل طلبية بشركتها/موصّلها، ولا يعرض ملخصًا للتأكيد، ولا يحترم فحص الجهوزية. **وبعد المراجعة:** المودال لم يميّز الجاهز عن غير الجاهز (كانت مجموعة «بلا شركة» تظهر كأنها ستُرسل ثم تُتخطى بصمت)، واسم الموصّل لم يُعرض اسمه الحقيقي، والتوست لم يذكر أرقام الطلبيات المتخطاة ولا أسبابها.
- **الحل النهائي:**
  - `$resolveBulkOrderState(Order)` — مصدر الحقيقة الوحيد: `carrierKey = shipping_provider_id ?: delivery_rider_id ?: 'unassigned'`، `carrierName` (اسم الشركة **أو اسم الموصّل الحقيقي** عبر relation `deliveryRider`، أو «بلا شركة»)، و`reasons[]` صريحة: تحتاج تأكيدًا/تجهيزًا (مع الحالة الحالية عبر `status_label`)، أو كل الحقول الناقصة (عبر `$collectMissingFields`)، أو بلا شركة/موصّل.
  - `$openBulkSendModal()` — جلب واحد بـ `with(['status','customer','shippingProvider','deliveryRider','items'])` (لا استعلامات N+1)؛ يملأ `bulkSendAnalysis`/`bulkSendReadyCount`/`bulkSendSkipCount`؛ ملخص `bulkSendSummary` من **الجاهزة فقط**.
  - مودال التأكيد قسمان: «ستُرسل» (ملخص الناقلين) + `<x-edz.alert type="warning">` يسرد كل طلبية غير جاهزة (رقم + سبب)؛ بلا جاهزة → زر معطَّل (`bulk_send_confirm_none`)؛ وإلا نص «إرسال :count الجاهزة فقط» (`bulk_send_confirm_some`).
  - `$confirmBulkSend()` — يعيد الجلب طازجًا بنفس eager-loads ويعيد الحساب بالمولّد نفسه (طزاجة + مصدر واحد)، يرسل الجاهزة فقط كلٌّ لناقلها الفعلي (لا auto-confirm أبدًا)، توست واحد: سطور الشركات + رأس `bulk_send_skipped :count` + سطر لكل طلبية `رقم (السبب)`.
  - `$sendConfirmedOrder` (29.2) يعيد استخدام `$collectMissingFields` نفسه (إزالة النسخ المكرر).
- **UI:** زر واحد «إرسال لشركة التوصيل» في شريط الإجراءات المشترك + مودال تأكيد (bottom-sheet تلقائيًا ≤639px حسب `_modal.scss`، صف أزرار `flex-col sm:flex-row`). درج «تأكيد وإرسال» (P26) لم يُعدَّل.
- **ترجمات:** مفاتيح 16 ×4 لغات في `order_flow.php`: مفاتيح 9 القائمة + 7 جديدة: `bulk_send_ready_title`/`bulk_send_skipped_title`/`bulk_send_reason_status`/`bulk_send_reason_missing`/`bulk_send_reason_no_carrier`/`bulk_send_confirm_some`/`bulk_send_confirm_none`.
- **ملاحظة اختبار:** `assertDispatched('swal:toast', closure)` يفحص أول حدث مطابق فقط — لذا توست واحد يجمع النتائج.
- **أدلة:** `tests/Feature/Merchant/BulkSendCarrierGroupingTest.php` — **9 ناجحة (48 assertions)**: منع بلا صلاحية بتوست `messages.unauthorized` (كان 403؛ نُقِّح في إغلاق 29.7)، تحذير بدون تحديد، تصنيف المودال (مجموعات جاهزة بأسماء الناقلين الفعلية + أعلام غير الجاهزة بأسبابها + `assertSee` للأرقام والأسباب في المودال)، إرسال كل طلبية لناقلها مع ملخص، تخطِّي pending دون تأكيد تلقائي **مع ذِكر رقمها صراحةً**، تخطِّي الناقص **مع ذِكر رقمه والحقل الناقص**، إرسال طلبية موصّل فقط **باسم الموصّل**، تخطِّي بلا شركة مع السبب، «لا شيء جاهز» → تحذير صريح دون أي إرسال. السويت كاملة **314 ناجح (1148 assertions)** → **صفر انحدار**. `view:cache` + `php -l` سليمان.

### 29.4 — عمود حالة التتبع + توحيد الخط الزمني للأحداث (واجهة خالصة، بموافقة المستخدم؛ بلا بذر محلي بقرار منه)

- **الفجوة:** جدول الطلبيات لا يعرض حالة التتبع للطلبيات المشحونة، وقسم «سجل الطلبية» (الخط الزمني) مكرَّر حرفيًا في درج التفاصيل ودرج التتبع.
- **الحل:**
  - عمود «حالة التتبع» ثانوي (`group: workflow`, `default: false`) في `$orderColumns()` — لا يظهر افتراضيًا (ضبط بسيط من إعدادات الجدول)؛ `interval tracking_status` في بانيّ `$arr['tracking']` الأول (الجدول) وfallback `openOrderDetails`؛ خلية الجدول تصفّ شارة mystatuskit بنطاق `tracking` (لدى الحالة) أو `—`؛ بطاقة الموبايل تجد شارة بجانب شارة الحالة عند تفعيل العمود فقط.
  - توحيد الخط الزمني: `partials/order-events-timeline.blade.php` (بـ `@include` تمرّر `events` + `icon`) يحل محل الكتلة في `orders/index.blade.php` (`canViewOrderDetailsEvents` + `detailsEvents`, icon `clock`) وفي `tracking/index.blade.php` (`canViewDrawerEvents` + `drawerEvents`, icon `list-bullet`). الحرّاسات خارج الجزئية — صلاحيات 29.1 سليمة.
  - مفتاح واحد `merchant_panel.tracking_status` ×4 لغات (رأس الجدول + منتقي الأعمدة). لا مفاتيح جديدة في `order_flow`.
- **قرار:** بذرة `order_tracking_histories` المحلية **ملغاة** بتوجيه صريح من المستخدم.
- **أدلة:** `tests/Feature/Merchant/OrdersTrackingColumnTest.php` — **3 ناجحة**: شارة تتبع مع تفعيل العمود، غياب الشارة افتراضيًا، `—` عند عدم وجود شحنة (بعد تعارض محنة: حالة `damaged` خاصة بنطاق `tracking`، بينما `in_transit` موجودة في نطاق `order` أيضًا). الاختبارات المرتبطة بالجزئية: `OrderEventLogVisibilityTest` (8) قائمة تغطي الدرجين بعد التوحيد. السويت كاملة **317 ناجح (1158 assertions)** → **صفر انحدار**. `view:cache` + `php -l` سليمان.
- **متابعة بناءً على ملاحظة المستخدم:** كانت حراسة زري الإرسال في الجدول وبطاقة الموبايل تقرأ `$order['status']['key']` (عنوان متداخل كُهرب عبر `toArray()` لعلاقة `status`). لعدم أمانة هذا الشكل (يُقرأ خطأً كعمود مباشر في `orders`)، أضيف `$arr['status_key'] = $order->status?->key` في بانيّ الصف (`loadOrders`) — مستمدًّا من جدول الحالات عبر مفتاح `status_id` — وأصبح الحارسان `in_array($order['status_key'] ?? null, ...)`. لا تغيير سلوكي؛ السويت **317 ناجح (1158 assertions)**.

### 29.5 — شارة التكرار + الفحص البطيء (بموافقة المستخدم)

- **العلامة (غير معطِّلة):** استعلام تجميعي واحد في `loadOrders` على نافذة 30 يومًا — `COUNT(*)` حسب `customer_id` (`havingRaw >= 2`) — تستهلك فهرس `orders_duplicate_scan`؛ البانيّ يضيف `$arr['duplicate_count']`. الشارة `edz-badge--warning --sm` «×N» في خلية الرقم (desktop) ورأس البطاقة (موبايل) مشروطة بـ `!showTrash` + `status_key !== 'duplicate'` + `count ≥ 2`؛ لا مفاتيح ترجمة جديدة. سبب `customer_id` (وليس phone): `store+phone` فريد ⇒ تطابق الهاتف == نفس العميل، والنافذة/النطاق مطابقان لشرط `findSimilar` (استثناء نفسه فقط).
- **الفحص البطيء (لاحق، لا محسوب):** `$openDuplicateScan(orderId)` → `findSimilar($order)` عند النقر، ويعرض `<x-edz.modal>` بمحتوى درج التأكيد (ستايله: رقم • منذ…، ×كمية، زر انتقال لدرج التفاصيل) مع `@edz-modal-closed.window`. بلا `wire:lazy` (غير موجود بالكود) — «البطيء» = عند الطلب بنمط المودالات القائمة. النافذة معلومات لكل أعضاء الصفحة (أرقام متجر نفسه مرئية أصلًا) و`markOrderDuplicate` (ORDER_MANAGE) خارج النطاق.
- **أدلة:** `tests/Feature/Merchant/OrderDuplicateBadgeTest.php` — **5 ناجحة (11 assertions)**: ① شارة الصفّين المكرّرين، ② لا شارة لطلبية وحيدة، ③ النقر يعرض الآخرين فقط (نتيجتان بلا نفسه + رقم ظاهر)، ④ حالة `duplicate` بلا شارة، ⑤ خارج النافذة (35–40 يوم) بلا شارة. ملاحظة تقنية: `created_at` محجوب في `create()` فتعديله بعد الإنشاء بصفة الحالة (خلاف ذلك يُركّب الآن()). السويت **322 ناجح (1169 assertions)** — صفر انحدار؛ `view:cache` + `php -l` سليمان.
- **متابعة 29.5a — توحيد تعريف الكشف (الرقم + المنتج) — تنفيذ بموافقة المستخدم:** ① سبب الفراغ مضبوط بالبيانات الحية: 00005/00006 (0550669633075) منتجان مختلفان ⇒ علامة نعم لكن `findSimilar` (رقم+منتج) صفر. ② `countsBySiblings($orderIds, $storeId)` — استعلامان (طلبيات النافذة + أصنافها، فهرس `orders_duplicate_scan`) → `same_phone`/`same_product`؛ الشارة = **`same_product`** فقط (سقف عرض 9+) وشرطها `>=1`؛ لا ترشيح حالة (مطابق لـ `findSimilar`). ③ `findSimilar`: `->with(['status','items'])` (إزالة N+1). ④ حالة الفراغ: `duplicateScanPhoneCount` في `openDuplicateScan`؛ إن عرضت المنتجات المختلفة → رسالة محايدة بمفتاح واحد جديد `duplicate_phone_only_orders` ×4 بدل «لا يوجد مكرر» الأخضر. تحقّق: **323 ناجح (1177 assertions)** — صفر انحدار.
- **29.6 — التصنيف الثلاثي بجانب الاسم (قرارات المستخدم: كشف فوري + تأكيد بضغطة لا كتابة حالة تلقائية؛ «سبق الطلب» = طلبية سابقة وصلت الناقل بأي عمر):** `countsPriorCarrierOrders()` (عدّ بلوغ الناقل: provider/rider أو حالة carrier()؛ استثناء الذات في المتصل). `dup_level` أولوية واحدة للصف (duplicate > probable > repeat). الوشم بجانب الاسم desktop/mobile وحذفه من الرقم. النافذة: شريحة تصنيف + سطر تاريخ الناقل الدائم. اختبارات **10** → السويت **326 ناجح (1204 assertions)**. ملاحظة: مفتاح الإرسال `shipped` لا `sent`؛ وتسميات مترجمة تصطدم مع نطاق الحالات (الفحص بنغمات edz-badge).
- **29.7 — «التروس → توست» + تكافؤ بطاقة الموبايل (الاثنان معًا):** ① كل توست الطلبيات انتقل لقناة `swal:toast` (payload `['icon'=>…,'title'=>…]`): bulk assign/delete/restore/trash، transitionOrder، reassign، delete، حارس shipped، out_of_stock/max_qty/insufficient_stock، create/update؛ وترجمة الحرفيّات (`select_agent`، `messages.unauthorized`، مفاتيح `merchant.order_*` الجديدة ×4، `cannot_edit_shipped`، `send_failed` بدل `$e->getMessage()` مع بقاء Log). ② قائمة الانتقالات في شارة حالة البطاقة (x-data المشترك `openStatusMenu()`) + تحرير الهاتف/الولاية inline في البطاقة بمفاتيح `phone-inline-card-`/`wilaya-inline-card-` (دوائر avoid-wire:key). ③ **إغلاق توست-403 (أمر «أكمل كل ما يتعلق بـ29»):** حواريز تدفق الإرسال الأربعة (`sendConfirmedOrder`/`submitConfirmAndSend`/`openBulkSendModal`/`confirmBulkSend`) من `abort_unless(...403)` إلى توست `messages.unauthorized` (الوعد القديم «توست بدل 403» في 29.2)؛ بقية الحواريز تبقى 403. اختبارات **11** جديدة + ترحيل 6 تأكيدات + تنقيح اختبارَي الإرسال (403→توست) → السويت **337 ناجح (1224 assertions)**. ملاحظة: معلومة الاختبارات — التوست مرصوص كمعامل واحد فقرينة الفحص `$params[0]['icon']`.
- **تذييل الجولة 2026-09-06 — بنود البرومت ذي الترقيم المحلّي (29.4 سجل التتبع نافذة + سجل أحداث الطلبية قائمة/نافذة، 29.5 فحص عدد الاستعلامات، 29.6 توحيد الحرّاس توست بدل 403، 29.7 قائمة «المزيد» المتنقلة بأهداف 44px):** نُفِّذت كلها فوق المحتوى السابق — صفحة الطلبيات: `order-events-menu` + `order-events-modal` (سجل أحداث الطلبية، حُرر من الدرج)؛ صفحة التتبع: `tracking-history-popup` (نافذة سجل التتبع من شارة/زر نطاق tracking)؛ `OrdersPageQueryCountTest` يعدّ استعلامات البيانات فقط؛ حواريز orders index كلها `permission_denied` توست (بقي 403 لمستوى `mount`) + `HasInlineEdit::saveEdit`؛ `orderMoreMenu` بصفوف `min-h-[44px]` في بطاقة الموبايل. السويت **348 ناجح (1268 assertions)** — صفر انحدار؛ `npm run build` + `view:cache` سليمان. التفاصيل في `Todos.md` (جولة التنفيذ) و`OrdersrefactorplanFixed.md` (تذييل 2026-09-06).
