                ...
                <li class="flex items-center {{ !$loop->last ? 'flex-1' : '' }}">
                    <div class="flex items-center gap-2">
                        <span class="flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold
                            {{ $isComplete ? 'bg-success-500 text-white' : ($isCurrent ? 'bg-brand-500 text-white ring-4 ring-brand-100 dark:ring-brand-900/40' : 'bg-surface-secondary text-ink-muted border border-surface-border') }}">
                            @if ($isComplete)
