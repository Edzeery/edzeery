...
mount(function (): void {
    ...
    $this->allStatuses = Status::where('type', 'order')
        ->where(fn ($q) => $q->whereNull('store_id')->orWhere('store_id', $storeId))
        ->orderBy('sort_order')
        ->get()
        ->toArray();

    $this->allMembers = StoreMembership::where('store_id', $storeId)
...
$loadOrders = function (): void {
    ...
    $query = Order::where('store_id', $storeId)
        ->with(['customer', 'status', 'items.product', 'items.variant', 'assignedMembership.user', 'createdByMembership.user', 'state', 'city']);
    ...
    if (! empty($f['phone'])) {
        $query->where(function ($q) use ($f) {
            $q->whereHas('customer', fn ($q2) => $q2->where('phone', 'like', "%{$f['phone']}%"))
              ->orWhere('phone_secondary', 'like', "%{$f['phone']}%");
        });
    }
    ...
    $this->orders['data'] = $paginated->getCollection()->map(function ($order) use ($service) {
        $arr = $order->toArray();
        $arr['transitions'] = $service->availableTransitions($order);
        $arr['items_summary'] = $order->items->map(fn ($i) => [
...
$clearFilters = function (): void {
    $this->filters = [
        'number' => '', 'customer' => '', 'phone' => '', 'status' => [],
        'wilaya' => null, 'assigned_to' => null, 'amount_min' => null,
        'amount_max' => null, 'date_from' => null, 'date_to' => null,
    ];
    $this->page = 1;
    $this->loadOrders();
};
...
$openCreateModal = function (): void {
    ...
    $this->form = [
        'customer_name' => '', 'customer_phone' => '', 'phone_secondary' => '',
        'address' => '', 'state_id' => '', 'city_id' => '', 'delivery_type' => 'home',
        'shipment_type' => 'delivery', 'payment_method' => 'cod', 'notes' => '', 'weight_kg' => '',
        'items' => [],
    ];
...
<div x-data="{ openFilter: null, openColToggle: false }">
            ...
            <div class="relative" @click.away="openFilter = null">
                <button @click="openFilter = openFilter === 'status' ? null : 'status'"
                    ...
                    @if(!empty($this->filters['status']))
                        <span class="ml-1 px-1.5 py-0.5 text-[10px] rounded-full bg-accent-100 text-accent-700">{{ count($this->filters['status']) }}</span>
                    @endif
                </button>
                <div x-show="openFilter === 'status'" x-transition
                    ...
                    @foreach($this->allStatuses as $s)
                        <label class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer text-sm">
                            ...
                            <span class="w-2 h-2 rounded-full" style="background: {{ $s['color'] === 'gray' ? '#6b7280' : ($s['color'] === 'info' ? '#3b82f6' : ($s['color'] === 'success' ? '#22c55e' : ($s['color'] === 'warning' ? '#f59e0b' : '#ef4444'))) }}"></span>
                            {{ $s['label'] }}
                        </label>
                    @endforeach
                </div>
                            ...
                            <tbody class="divide-y divide-surface-100 dark:divide-ink-800">
                                @foreach($orders['data'] as $order)
                                    ...
                                    <tr class="hover:bg-surface-50 dark:hover:bg-ink-800/50">
                                        ...
                                        @if(in_array('customer', $this->visibleColumns))
                                            <td class="px-4 py-3">
                                                <div class="text-ink font-medium">{{ $order['customer']['name'] ?? '-' }}</div>
                                            </td>
                                        @endif
                                        @if(in_array('phone', $this->visibleColumns))
                                                    ...
                                                    <div x-show="open" x-transition
                                                        ...
                                                        @foreach($this->allStatuses as $s)
                                                            @if(in_array($s['key'], $transitions) || $s['id'] == $order['status_id'])
                                                                <button wire:click="transitionOrder('{{ $order['id'] }}', '{{ $s['key'] }}')"
                                                                        @click="open = false"
                                                                        class="w-full text-left flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs hover:bg-gray-50 dark:hover:bg-gray-700 {{ $s['id'] == $order['status_id'] ? 'font-bold' : '' }}">
                                                                    <span class="w-2 h-2 rounded-full shrink-0" style="background: {{ match($s['color'] ?? 'gray') { 'success' => '#22c55e', 'info' => '#3b82f6', 'warning' => '#f59e0b', 'danger' => '#ef4444', default => '#6b7280' } }}"></span>
                                                                    {{ $s['label'] }}
                                                                </button>
                                                            @endif
                                                        @endforeach
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex items-center justify-end gap-1">
                                                <button wire:click="toggleDetail('{{ $order['id'] }}')"
                                                        ...
                                                        :class="$expandedOrderId === '{{ $order['id'] }}' ? 'text-primary-500' : ''">
                                                    <ion-icon name="{{ $expandedOrderId === $order['id'] ? 'chevron-up' : 'chevron-down' }}-outline" class="text-sm"></ion-icon>
                                                </button>
                                                @if(canStore(\App\Enums\Store\StorePermissionEnum::ORDER_MANAGE->value))
                                                    ...
                                                    <button wire:click="openReassignModal('{{ $order['id'] }}')"
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                                                    <div>
                                                        ...
                                                        @foreach($order['items_summary'] ?? [] as $item)
                                                            <div class="flex justify-between py-1 border-b border-surface-200 dark:border-ink-700">
                                                                <span class="text-ink-muted">{{ $item['name'] }} ×{{ $item['qty'] }}</span>
                                                                <span class="font-medium text-ink">{{ currency($item['price'] * $item['qty']) }}</span>
                                                            </div>
                                                        @endforeach
                                                    ...
                                                    <div>
                                                        ...
                                                        <dl class="space-y-1 text-ink-muted">
                                                            <div class="flex justify-between"><dt>Delivery:</dt><dd class="text-ink capitalize">{{ $order['delivery_type'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Shipment:</dt><dd class="text-ink capitalize">{{ $order['shipment_type'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Payment:</dt><dd class="text-ink uppercase">{{ $order['payment_method'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Weight:</dt><dd class="text-ink">{{ $order['weight_kg'] ? $order['weight_kg'] . ' kg' : '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Phone 2:</dt><dd class="text-ink">{{ $order['phone_secondary'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Address:</dt><dd class="text-ink">{{ $order['address'] ?? '—' }}</dd></div>
                                                        </dl>
                                                    </div>
                                                    <div>
                                                        <h4 class="font-semibold text-ink mb-2">Assignment</h4>
                                                        <dl class="space-y-1 text-ink-muted">
                                                            <div class="flex justify-between"><dt>Agent:</dt><dd class="text-ink">{{ $order['assigned_membership']['user']['name'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Method:</dt><dd class="text-ink capitalize">{{ $order['assignment_method'] ?? '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Created by:</dt><dd class="text-ink">{{ $order['created_by_membership_id'] ? ($order['created_by_membership']['user']['name'] ?? '—') : '—' }}</dd></div>
                                                            <div class="flex justify-between"><dt>Attempts:</dt><dd class="text-ink">{{ $order['confirmation_attempts'] ?? 0 }}</dd></div>
                                                            <div class="flex justify-between"><dt>Last contact:</dt><dd class="text-ink">{{ $order['last_contact_at'] ? \Carbon\Carbon::parse($order['last_contact_at'])->diffForHumans() : '—' }}</dd></div>
                                                            @if(!empty($order['notes']))
                                                                <div class="mt-2 p-2 bg-surface-100 dark:bg-ink-700 rounded-lg text-ink-muted italic">"{{ $order['notes'] }}"</div>
                                                            @endif
                                                        </dl>
                                                    </div>
                                                </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex gap-1">
                                    @if($orders['current_page'] > 1)
                                        <button wire:click="$setPage({{ $orders['current_page'] - 1 }})" class="edz-btn edz-btn--ghost edz-btn--xs">&laquo;</button>
                                    @endif
                                    ...
                                    @if($orders['current_page'] < $orders['last_page'])
                                        <button wire:click="$setPage({{ $orders['current_page'] + 1 }})" class="edz-btn edz-btn--ghost edz-btn--xs">&raquo;</button>
                                    @endif
                                </div>
                            </div>
                        @endif
    ...
    @if($showCreateModal || $showEditModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data>
            <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" x-on:click="$wire.set('{{ $showEditModal ? 'showEditModal' : 'showCreateModal' }}', false)"></div>
            <div class="relative w-full max-w-2xl bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto">
                <div class="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between z-10">
                    <h3 class="text-lg font-bold text-ink">{{ $showEditModal ? __('Edit Order') : __('New Order') }}</h3>
                    <div class="flex items-center gap-2">
                        <button wire:click="{{ $showEditModal ? 'submitEdit' : 'submitCreate' }}" class="edz-btn edz-btn--primary edz-btn--sm">
                            {{ $showEditModal ? __('Update') : __('Create') }}
                        </button>
                        <button type="button" class="edz-btn edz-btn--ghost edz-btn--sm"
                                x-on:click="$wire.set('{{ $showEditModal ? 'showEditModal' : 'showCreateModal' }}', false)">
                            ...
                            <div class="space-y-2 mt-2">
                                @foreach($form['items'] as $idx => $item)
                                    <div class="flex items-center gap-3 p-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-sm">
                                        ...
                                        <span class="text-ink-muted w-20 text-right">{{ currency($item['price'] * $item['quantity']) }}</span>
                                        <button wire:click="removeFormItem({{ $idx }})" class="text-red-400 hover:text-red-600">
                                            <ion-icon name="close-outline"></ion-icon>
                                        </button>
                                    </div>
                                @endforeach
                                <div class="text-right font-bold text-ink pt-2 border-t">
                                    Total: {{ currency(collect($form['items'])->sum(fn($i) => $i['price'] * $i['quantity'])) }}
                                </div>
                            </div>
                        @endif
                    </div>
            ...
            <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-2xl">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" class="edz-btn edz-btn--ghost edz-btn--sm" x-on:click="$wire.set('showReassignModal', false)">{{ __('Cancel') }}</button>
                        <button wire:click="submitReassign" class="edz-btn edz-btn--primary edz-btn--sm">{{ __('Reassign') }}</button>
                    </div>
                </div>
            </div>
        </div>
