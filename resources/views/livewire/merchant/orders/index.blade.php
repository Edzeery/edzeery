...
mount(function (): void {
    ...
    $this->searchableStatuses = collect($this->allStatuses)
        ->map(fn ($s) => [
            'id' => $s['id'],
            'name' => \Edzeery\MyStatusKit\Facades\Status::for('order', $s['key'] ?? 'default')->label(),
            'color' => $s['color'] ?? 'gray',
        ])
        ->values()
        ->all();
    ...
    $this->searchableMembers = collect($this->allMembers)
        ->map(fn ($m) => [
            'id' => $m['id'],
