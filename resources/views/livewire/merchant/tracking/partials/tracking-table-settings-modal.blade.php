{{-- Tracking grid settings modal (advanced grid) — per-tab columns + style, mirroring the orders page.
    Opens from tracking-toolbar (openTableSettings); columns tab uses orderColumnReorderDraft. --}}

@if ($this->showTableSettings)
    <div @edz-modal-closed.window="$wire.discardTableSettings()">
        <x-edz.modal :isOpen="true" size="lg" wire:key="tracking-table-settings">
            <div x-data="{ tab: 'columns' }" class="p-6">
                ...
                <div x-show="tab === 'columns'" x-cloak class="space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-ink-muted mb-2">{{ __('merchant_panel.drag_to_reorder') }}</p>
                                    ...
                                    <table class="w-full">
                                        <tbody>
                                            <tr class="edz-table-row--success">
                                                ...
                                                <td class="px-2.5 py-1.5">{{ __('order_flow.tracking_status') }}</td>
                                            </tr>
