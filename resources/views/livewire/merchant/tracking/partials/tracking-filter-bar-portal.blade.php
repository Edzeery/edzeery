{{-- Tracking toolbar drill-down filter portal (products-style). A single "Filters"
    ...
    root groups → options per group. Every filter is offered here regardless of
    column visibility (visible columns additionally keep their header filter icons). --}}
...
<div x-data="dropdownPosition()" x-show="open" @click.away="close()"
    ...
    <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
        ...
        class="fixed inset-x-0 bottom-0 z-[210] w-full rounded-t-2xl border border-b-0 border-surface-border bg-surface
               ...
               {{-- Taller scrollable area for large option lists (58 wilayas, 1540 communes). --}}
               sm:max-h-[70vh]
        ...
        @if (in_array('state', $available))
            <div x-show="open === 'state'" x-cloak class="edz-dropdown__section">
                <div class="mb-1 flex items-center gap-0.5 px-0.5">
                    ...
                    <p class="edz-dropdown__section-title m-0">{{ __('merchant_panel.state') }}</p>
                </div>

                <button type="button" @click="$wire.setFilter('state', null); close()"
                    aria-pressed="{{ empty($this->filters['state']) ? 'true' : 'false' }}"
                    ...
                    <x-edz.icon name="check"
                        class="w-3.5 h-3.5 shrink-0 {{ empty($this->filters['state']) ? 'opacity-100' : 'opacity-0' }}" />
                </button>
        ...
        @if (in_array('tracking_statuses', $available))
            <div x-show="open === 'tracking_statuses'" x-cloak class="edz-dropdown__section">
                ...
                @foreach ($this->trackingStatusOptions() as $opt)
                    <button type="button" @click="$wire.toggleTrackingStatus('{{ $opt['value'] }}'); close()"
                        ...
                        <x-edz.icon name="check"
                            class="w-3.5 h-3.5 shrink-0 {{ in_array($opt['value'], $this->filters['tracking_statuses'] ?? [], true) ? 'opacity-100' : 'opacity-0' }}" />
                    </button>
                @endforeach
            </div>
        @endif
        ...
        @if (in_array('products', $available))
            <div x-show="open === 'products'" x-cloak class="edz-dropdown__section">
                <div class="mb-1 flex items-center gap-0.5 px-0.5">
                    ...
                    <p class="edz-dropdown__section-title m-0">{{ __('merchant_panel.products') }}</p>
                </div>

                <button type="button" @click="$wire.setFilter('products', []); close()"
                    aria-pressed="{{ empty($this->filters['products']) ? 'true' : 'false' }}"
                    class="edz-dropdown__item justify-between {{ empty($this->filters['products']) ? 'bg-accent-surface text-accent-fg font-semibold' : '' }}">
                    <span class="truncate">{{ __('general.all') }}</span>
                    <x-edz.icon name="check"
                        class="w-3.5 h-3.5 shrink-0 {{ empty($this->filters['products']) ? 'opacity-100' : 'opacity-0' }}" />
                </button>
        ...
        @if (in_array('shipment_type', $available))
            <div x-show="open === 'shipment_type'" x-cloak class="edz-dropdown__section">
                ...
                <button type="button" @click="$wire.setFilter('shipment_type', null); close()"
                    aria-pressed="{{ empty($this->filters['shipment_type']) ? 'true' : 'false' }}"
                    class="edz-dropdown__item justify-between {{ empty($this->filters['shipment_type']) ? 'bg-accent-surface text-accent-fg font-semibold' : '' }}">
                    <span class="truncate">{{ __('general.all') }}</span>
                    <x-edz.icon name="check"
                        class="w-3.5 h-3.5 shrink-0 {{ empty($this->filters['shipment_type']) ? 'opacity-100' : 'opacity-0' }}" />
                </button>
                @foreach (['delivery' => __('merchant_panel.delivery'), 'exchange' => __('merchant_panel.exchange_label'), 'pickup' => __('merchant_panel.pickup_label')] as $val => $lbl)
                    <button type="button" @click="$wire.setFilter('shipment_type', '{{ $val }}'); close()"
                        ...
                        <span class="truncate">{{ $lbl }}</span>
                        <x-edz.icon name="check"
                            class="w-3.5 h-3.5 shrink-0 {{ ($this->filters['shipment_type'] ?? null) === $val ? 'opacity-100' : 'opacity-0' }}" />
                    </button>
                @endforeach
            </div>
        @endif
