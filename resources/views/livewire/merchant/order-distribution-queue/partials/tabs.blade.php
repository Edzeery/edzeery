...
<div class="flex gap-1 mb-6 border-b border-surface-border">
    <button wire:click="setTab('confirmation')"
        ...
        <span class="inline-flex items-center justify-center min-w-5 h-5 px-1.5 text-[11px] font-semibold rounded-full tabular-nums {{ $tab === 'confirmation' ? 'bg-brand-500/15 text-brand-700' : 'bg-surface-tertiary text-ink-muted' }}">
            {{ $confirmationCount }}
        </span>
    </button>
    ...
    <button wire:click="setTab('tracking')"
        ...
        <span class="inline-flex items-center justify-center min-w-5 h-5 px-1.5 text-[11px] font-semibold rounded-full tabular-nums {{ $tab === 'tracking' ? 'bg-brand-500/15 text-brand-700' : 'bg-surface-tertiary text-ink-muted' }}">
            {{ $trackingCount }}
        </span>
    </button>
</div>