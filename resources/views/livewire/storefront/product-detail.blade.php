    ...
    @if ($this->product)
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-8">
                <a href="/"
                    class="hover:text-gray-700 dark:hover:text-gray-200 transition">{{ $this->product->store->name ?? __('storefront.back_to_store') }}</a>
                <span>/</span>
                @if ($this->product->categories->first())
                    <span
                        class="hover:text-gray-700 dark:hover:text-gray-200 transition">{{ $this->product->categories->first()->name }}</span>
                    <span>/</span>
                @endif
                ...
                <div x-data="productGallery()" class="space-y-4" dir="ltr">
                    @if ($this->product->images->count())
                        ...
                        <div x-ref="mainGallery"
                            ...
                            aria-label="{{ __('storefront.product_images') ?? 'Product images' }}">
                            ...
                            @if ($__imageDiscountPercent > 0)
                                <span
                                    class="absolute top-4 {{ isRTL() ? 'start-4' : 'end-4' }} z-10 bg-red-500 text-white text-xs font-bold px-2.5 py-1.5 rounded-full shadow-lg">
                                    -{{ $__imageDiscountPercent }}%
                                </span>
                            @endif
                    ...
                    <div x-show="lightbox" x-transition.opacity
                        ...
                        @endif
                        @foreach ($this->product->images as $i => $img)
                            <img x-show="active === {{ $i }}" x-transition
                                src="{{ asset('storage/' . $img->path) }}" alt="{{ $this->product->name }}"
                ...
                <div class="flex flex-col"
                    ...
                    <div class="mb-4">
                        ...
                        <template x-if="!activeVariant">
                            <div class="flex items-center flex-wrap gap-x-3 gap-y-1">
                                <span
                                    class="text-3xl font-bold store-text-primary">{{ currency($__minVariantPrice ?: $this->product->price) }}</span>
                                @if ($__imageDiscountPercent > 0 && $__maxComparePrice > 0)
                                    <span
                                        class="text-lg text-gray-400 dark:text-gray-500 line-through">{{ currency($__maxComparePrice) }}</span>
                                    <span
                                        class="text-xs font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded-full">-{{ $__imageDiscountPercent }}%</span>
                                @endif
                            </div>
                        </template>
                    </div>
                    ...
                    @if ($this->product->short_description)
                        <p class="text-gray-600 dark:text-gray-300 leading-relaxed mb-6">
                            {{ $this->product->short_description }}</p>
                    @endif
                    ...
                    @if ($this->product->variants->count() > 1)
                        <div class="mb-6">
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ __('storefront.options') }}</label>
                            <div class="flex flex-wrap gap-2">
                                @foreach ($this->product->variants as $variant)
                                    ...
                                    <button type="button" x-on:click="select('{{ $variant->id }}')"
                                        ...
                                        @if ($__outOfStock)
                                            <span
                                                class="ml-1 text-xs font-semibold text-red-500 dark:text-red-400 no-underline">{{ __('storefront.out_of_stock_short') }}</span>
                                        @endif
                                    </button>
                                @endforeach
                            </div>
                    ...
                    @if (!$__initialOutOfStock)
                        <div class="mb-8">
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ __('storefront.quantity') }}</label>
                            <div
                    ...
                    <button type="button" wire:click="addToCart()" :disabled="isOutOfStock"
                        ...
                        <x-edz.icon name="shopping-cart" class="text-white text-bold text-xl w-5 h-5" />

                        <span x-text="isOutOfStock ? outOfStockShortLabel : addToCartLabel" wire:loading.remove=""
                            wire:target="addToCart">{{ __('storefront.add_to_cart') }}</span>
                        <span wire:loading class="hidden"
                            wire:target="addToCart">{{ __('storefront.placing') }}</span>
                    </button>

                    {{-- Trust signals --}}
                    <div class="mt-10 pt-8 border-t border-gray-200 dark:border-gray-700 grid grid-cols-3 gap-4">
                        <div class="flex flex-col items-center text-center">
                            <x-edz.icon name="shield-check" class="text-2xl store-text-primary mb-1 w-8 h-8" />
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ __('storefront.secure_payment') }}</span>
                        </div>
                        <div class="flex flex-col items-center text-center">
                            <x-edz.icon name="truck" class="text-2xl store-text-primary mb-1 w-8 h-8" />
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ __('storefront.fast_delivery') }}</span>
                        </div>
                        <div class="flex flex-col items-center text-center">
                            <x-edz.icon name="arrow-path" class="text-2xl store-text-primary mb-1 w-8 h-8" />
                            <span
                                class="text-xs text-gray-500 dark:text-gray-400">{{ __('storefront.easy_returns') }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if ($this->product->description)
                <div class="mt-16 pt-12 border-t border-gray-200 dark:border-gray-700">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">
                        {{ __('storefront.product_details') }}</h2>
                    <div
            ...
            @if ($this->relatedProducts->count())
                <section class="mt-16 pt-12 border-t border-gray-200 dark:border-gray-700">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-8">
                        {{ __('storefront.you_may_also_like') }}</h2>
                            ...
                            <div
                                ...
                                <div class="p-4 flex flex-col flex-1">
                                    ...
                                    <div class="flex items-center flex-wrap gap-x-2 gap-y-1 mt-2">
                                        <span
                                            class="text-lg font-bold store-text-primary">{{ currency($__rpMinPrice) }}</span>
                                        @if ($__rpDiscount > 0)
                                            <span
                                                class="text-xs font-medium text-gray-400 dark:text-gray-500 line-through">{{ currency($__rpMaxCompare) }}</span>
                                            <span
