                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div>
                            @if ($this->product->brand)
                                <span
                                    class="inline-block text-sm font-semibold store-text-primary mb-3 uppercase tracking-wider">{{ $this->product->brand->name }}</span>
                            @endif
                            <h1
                            ...
                            <div class="flex items-center flex-wrap gap-x-3 gap-y-1 mb-2">
                                <span
                                    class="text-2xl sm:text-3xl lg:text-4xl font-bold store-text-primary">{{ currency($_heroPrice) }}</span>
                                @if ($_heroShowCompare)
                                    <span
                                        class="text-lg text-gray-400 dark:text-gray-500 line-through">{{ currency($_heroCompare) }}</span>
                                    <span
                                        class="text-sm font-bold text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/30 px-2 py-0.5 rounded-full">-{{ $_heroPct }}%</span>
                                @endif
                            </div>
                            ...
                            @if ($_variant)
                                <p
                                    class="mb-6 text-sm font-medium {{ match ($_variant->stockStatus()) {
                                    ...
                                    <x-status-badge domain="product" status="{{$status}}" set="bi" />
                                </p>
                            @endif
                        ...
                        <div class="relative">
                            @if ($this->product->images->count())
                                <img src="{{ asset('storage/' . $this->product->images->first()->path) }}"
                            ...
                            @if ($this->product->is_featured)
                                <span
                                    class="absolute top-4 {{ isRTL() ? 'end-4' : 'start-4' }} z-10 flex items-center gap-1.5 store-bg-primary text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg">
            ...
            <section class="py-16 bg-white dark:bg-gray-800">
                <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6 text-center">
                        {{ $this->section_content['description']['title'] ?? __('storefront.product_details') }}</h2>
                    <div
            ...
            <section class="py-16 store-gradient">
                <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    <h2 class="text-3xl font-bold text-white mb-4">
                        {{ $cta['title'] ?? __('storefront.ready_to_order') }}</h2>
                    <p class="text-white/80 mb-8 text-lg">{{ $cta['description'] ?? __('storefront.get_yours_now') }}
                    </p>
